...
| F18 | **Split Settings into sub-screens** (Game Modes / Gameplay / Sound & Haptics) | M | Settings is getting crowded — currently 10+ rows plus a scrolling sound-pack list. Proposed grouping: **Game Modes** (Memory Lights+, Difficulty, Reverse Mode, Practice Mode), **Gameplay** (Player Timeout, plus future F3/F4/F6/F8 toggles), **Sound & Haptics** (Sound Pack list, Vibrate toggle — top-bar mute stays where it is), **Top-level Settings** keeps Statistics, Reset, Rate, About. **Pros:** less scroll, clearer mental model, room to grow as F3/F4/F6/F8 land; matches Android Material Settings convention (chevron-into-detail). **Cons:** extra tap to flip a toggle (power-user friction); requires nav refactor — current `ScreenState` is a flat sealed class (Game / Settings / Statistics), so either expand it with new cases or migrate to `androidx.navigation.compose.NavHost`. **NavHost is the cleaner long-term answer** — gets typed back-stack handling, per-screen lifecycle, deep-link readiness for free, and unblocks future sub-screens beyond just settings. **Sequencing:** do the NavHost migration as its own step before adding the sub-screens, so the refactor isn't tangled with new UI work. |
